package com.programareavansata.rest.helpers;
...
public class Gomoku {
    ...
    public Gomoku(Game game) {
        board = new int[W][H];
        this.game = game;
        final List<Row> list = game.getBoard();
        for (int i = 0; i < W; i++) {
            for (int j = 0; j < H; j++) {
                board[i][j] = list.get(i).getLine().get(j);
            }
        }
    }
    ...
    public Game getGame() {
        for (int i = 0; i < W; i++) {
            for (int j = 0; j < H; j++) {
                game.getBoard().get(i).getLine().set(j, board[i][j]);
            }
        }
        return game;
    }
